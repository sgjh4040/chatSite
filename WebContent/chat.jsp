...
<head>
	<%
		...
		if(toID == null){
			session.setAttribute("messageType", "오류 메시지");
			session.setAttribute("messageContent", "대화상태가 지정되지 않았습니다.");
			response.sendRedirect("index.jsp");
			return;
		}if(userID.equals(URLDecoder.decode(toID,"UTF-8"))){
			session.setAttribute("messageType", "오류 메시지");
			session.setAttribute("messageContent", "자기자신에게는 쪽지를 보낼수 없습니다.");
			response.sendRedirect("index.jsp");
			return;
		}
...
<script type="text/javascript">
		...
		function addChat(chatName,chatContent,chatTime){
			...
			$('#chatList').append('<div class="row">'+
								...
								'<img class="media-object img-circle"style="width:40px;"style="width:30px;height:30px;" src="<%=fromProfile%>" alt="">'+
								'</a>'+
				...
				$('#chatList').append('<div class="row">'+
						...
						'<div class="media">' +
						'<a class="pull-left" href="#">'+
						'<img class="media-object img-circle"style="width:40px;"style="width:30px;height:30px;" src="<%=toProfile%>" alt="">'+
						'</a>'+
						'<div class="media-body">'+
						'<h4 class="media-heading">'+
						chatName+
